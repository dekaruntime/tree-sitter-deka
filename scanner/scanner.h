...
enum TokenType {
    ...
    ERROR_RECOVERY,
    // DEKA-SPECIFIC: raw JavaScript body of an `unsafe { ... }` block.
    // Must stay last: it is appended after the tree-sitter-typescript
...
static bool scan_raw_js(TSLexer *lexer) {
    lexer->result_symbol = RAW_JS;
    int32_t brace_depth = 0;
    int32_t quote = 0;       // active string quote character, 0 when outside a string
    ...
    while (lexer->lookahead != 0) {
        ...
        switch (lexer->lookahead) {
            ...
            case '}':
                if (brace_depth == 0) {
                    // An empty raw block is handled by the grammar
                    // (raw_js is optional); a zero-width token would
                    // confuse the lexer.
                    lexer->mark_end(lexer);
                    return saw_content;
                }
    ...
    // Unterminated `unsafe` block: let the parser report the error.
    return false;
}

static inline bool external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    if (valid_symbols[RAW_JS]) {
        return scan_raw_js(lexer);
    }
